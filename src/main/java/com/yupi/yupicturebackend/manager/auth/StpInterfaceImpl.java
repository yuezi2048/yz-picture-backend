...
public class StpInterfaceImpl implements StpInterface {
    ...
    public List<String> getPermissionList(Object loginId, String loginType) {
        // 1. 不属于space权限，这里就返回无权限
        if (!StpKit.SPACE_TYPE.equals(loginType)) {
            return new ArrayList<>();
        }

        // 2.对于没有传递图片信息/空间信息，全部字段为空，视为真正的管理员对公共图库操作，返回管理员列表
        ...
        // 4. 判断SpaceUser部分，优先从上下文中获取 SpaceUser 对象，如果能直接拿到spaceUserId，必然是团队空间
        ...
        // 5. 如果不能直接找到 spaceUserId，尝试通过 spaceId 或 pictureId 获取 Space 对象并处理
        Long spaceId = authContext.getSpaceId();
        // 5.1 如果没有 spaceId，通过 pictureId 获取 Picture 对象和 Space 对象
        if (spaceId == null) {
